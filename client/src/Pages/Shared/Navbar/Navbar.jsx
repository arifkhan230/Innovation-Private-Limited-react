
import { useNavigate } from "react-router-dom";
import Container from "../../../Components/Container/Container";
import { PropTypes } from "prop-types";


const Navbar = ({ cart }) => {
    const navigate = useNavigate();
    const userName = localStorage.getItem("userName");
    const userImage = localStorage.getItem("userImage");

    const totalPrice = cart.reduce((total, item) => item.price + total, 0);

    const handleLogout = () => {
        localStorage.clear();
        navigate("/login")
    }

    return (
        <div className="bg-base-100">
            <Container>
                <div className="navbar py-4">
                    <div className="flex-1">
                        <a className="btn btn-ghost text-xl font-bold">INNOVATION</a>
                    </div>
                    <div className="flex-none">
                        <div className="dropdown dropdown-end">
                            <div tabIndex={0} role="button" className="btn btn-ghost  btn-circle">
                                <div className="indicator">
                                    <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M3 3h2l.4 2M7 13h10l4-8H5.4M7 13L5.4 5M7 13l-2.293 2.293c-.63.63-.184 1.707.707 1.707H17m0 0a2 2 0 100 4 2 2 0 000-4zm-8 2a2 2 0 11-4 0 2 2 0 014 0z" /></svg>
                                    <span className="badge badge-sm indicator-item">{cart.length}</span>
                                </div>
                            </div>
                            <div tabIndex={0} className="mt-3 z-[1] card card-compact dropdown-content w-52 bg-base-100 shadow">
                                <div className="card-body">
                                    <span className="font-bold text-lg">{cart.length} Products</span>
                                    <span className="text-info">Subtotal: $ {totalPrice}</span>
                                   
                                </div>
                            </div>
                        </div>
                        <div className="dropdown dropdown-end ml-2">
                            <div tabIndex={0} role="button" className="btn btn-ghost btn-circle ">
                                <div className=" rounded-full border border-purple-500">
                                    <img alt="Tailwind CSS Navbar component" src={userImage} />
                                </div>
                            </div>
                            <ul tabIndex={0} className="menu menu-sm dropdown-content mt-3 z-[1] p-2 shadow bg-base-100 rounded-box w-52 space-y-2">

                                <li><p className="text xl font-medium">{userName}</p></li>
                                <li><button onClick={handleLogout} className="btn btn-sm bg-purple-400 text-white">Logout</button></li>
                            </ul>
                        </div>
                    </div>
                </div>
            </Container>
        </div>
    );
};

Navbar.propTypes = {
    cart: PropTypes.array,
};


export default Navbar;